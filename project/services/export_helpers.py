...
def export_project_profitability_excel(request, project_id):
    ...
    report_data = [data] if isinstance(data, dict) else data
    columns = [
        'project_name', 'total_earnings', 'total_expenses', 'total_da_claimed',
        'net_profit', 'expense_percentage', 'profit_percentage'
    ]
    return export_to_excel(report_data, columns, filename=f'Project_{project_id}_Profitability.xlsx')
...
def export_da_claims_excel(request, project_id):
    ...
    return export_to_excel(data, columns, filename=f'Project_{project_id}_DA_Claims.xlsx')
...
def export_timesheet_earnings_excel(request, project_id):
    ...
    return export_to_excel(data, columns, filename=f'Project_{project_id}_Timesheet_Earnings.xlsx')
